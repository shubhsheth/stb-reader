...
class TestM3U:
    def test_m3u_starts_with_extm3u(self, tc):
        client, mock = tc
        mock.live_tv.get_genres.return_value = [
            Genre(id="1", title="News", alias="news", censored=False),
        ]
        mock.live_tv.get_channels.return_value = _paged([
            Channel(id="10", number="1", name="CNN", cmd="x", logo="http://l.png", genre_id="1", hd=False, censored=False),
        ])
        resp = client.get(f"/get.php?{CREDS}&type=m3u_plus")
        assert resp.status_code == 200
        assert resp.text.startswith("#EXTM3U")

    def test_m3u_contains_channel_entry(self, tc):
        client, mock = tc
        mock.live_tv.get_genres.return_value = [
            Genre(id="1", title="News", alias="news", censored=False),
        ]
        ...
        text = client.get(f"/get.php?{CREDS}&type=m3u_plus").text
        assert "CNN" in text
        assert "#EXTINF" in text
        assert "group-title" in text
        assert "/testuser/testpass/10.m3u8" in text

    def test_m3u_auth_fail(self, tc):
        client, _ = tc
        resp = client.get(f"/get.php?{BAD_CREDS}&type=m3u_plus")
        assert resp.status_code == 403

    def test_m3u_empty_portal(self, tc):
        client, mock = tc
        mock.live_tv.get_genres.return_value = []
        mock.live_tv.get_channels.return_value = _paged([])
        resp = client.get(f"/get.php?{CREDS}&type=m3u_plus")
        assert resp.status_code == 200
        assert resp.text.strip() == "#EXTM3U"
...
class TestXMLTV:
    def test_xmltv_returns_200_xml(self, tc):
        client, _ = tc
        resp = client.get(f"/xmltv.php?{CREDS}")
        assert resp.status_code == 200
        assert "application/xml" in resp.headers["content-type"]
        assert "<tv" in resp.text
        assert resp.text.startswith("<?xml")

    def test_xmltv_auth_fail(self, tc):
        client, _ = tc
        resp = client.get(f"/xmltv.php?{BAD_CREDS}")
        assert resp.status_code == 403
